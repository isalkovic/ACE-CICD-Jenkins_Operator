...
setdbparms=$(base64 server-config/initial-config/setdbparms/setdbparms2.txt)
...
#truststore=$(base64 -d 0 server-config/initial-config/truststore/es-cert.p12)
...
policy=$(base64 server-config/initial-config/policy/policy.zip)
...
serverconf=$(base64 server-config/initial-config/serverconf/server.conf.yaml)
